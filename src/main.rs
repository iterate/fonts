...
impl TryFrom<Vec<u8>> for TableDirectoryEntry {
    ...
    fn try_from(value: Vec<u8>) -> Result<Self, Self::Error> {
        ...
        if tag != "name" {
            return Err(eyre!("Not name table entry"));
        }
...
fn main() -> Result<()> {
    // Reads entire file into 1-byte vector
    let content = fs::read("test_font_1.woff").expect("wtf"); // reads entire file into mem. Maybe use File::open to open with buffer?

    let signature = std::str::from_utf8(&content[0..4])?;

    if signature != "wOFF" {
        return Err(eyre!("Not woFF"));
    }
